...
public class JCommander
{
	...
	/**
	 * @param object
	 *            The arg object expected to contain {@link Parameter}
	 *            annotations.
	 */
	 ...
	 */
	public JCommander(Object object, @Nullable
	 ...
	 * @param object
	 *            The arg object expected to contain {@link Parameter}
	 *            annotations. If <code>object</code> is an array or is
	 *            {@link Iterable}, the child objects will be added instead.
	 */
	...
	private void addDescription(Object object)
	{
		...
		// f.getAnnotation(DynamicParameter.class);
		// if (annotation != null) {
		// //
		// // @Parameter
		// //
		...
		// throw new
		// ParameterException("Only one @Parameter with no names attribute is"
		...
		// for (String name : p.names()) {
		// if (m_descriptions.containsKey(name)) {
		// throw new ParameterException("Found the option " + name +
		// " multiple times");
		// }
		// p("Adding description for " + name);
		...
		// if (delegateObject == null){
		// throw new ParameterException("Delegate field '" + f.getName() +
		// "' cannot be null.");
		// }
		// addDescription(delegateObject);
	...
	/**
	 * Main method that parses the values and initializes the fields
	 * accordingly.
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void parseValues(String[] args, boolean validate)
	{
		// This boolean becomes true if we encounter a command, which indicates
		// we need
		...
		while (i < args.length && !commandParsed)
		{
			...
			if (isOption(args, a) && jc == null)
			{
				...
				if (pd != null)
				{
					...
					{
						//
						// Password option, use the Console to retrieve the
						// password
						//
					...
					else
					{
						...
						else
						{
							...
							// Boolean, set to true as soon as we see it, unless
							// it specified
							// an arity of 1, in which case we need to read the
							// next value
			...
			else
			{
				...
				if (!Strings.isStringEmpty(arg))
				{
					if (m_commands.isEmpty())
					{
						...
						String value = a; // If there's a non-quoted version,
											// prefer that one
					...
					else
					{
						...
						if (jc == null && validate)
						{
							...
							if (jc != null)
							{
								...
								m_parsedAlias = arg; // preserve the original
														// form

								// Found a valid command, ask it to parse the
								// remainder of the arguments.
								// Setting the boolean commandParsed to true
								// will force the current
								// loop to end.
	...
	/**
	 * @return The internationalized version of the string if available,
	 ...
	 *         target class. This can be used to display the usage() in a
	 *         different format (e.g. HTML).
	 */
	...
	public Object convertValue(Parameterized parameterized, Class type, String value)
	{
		...
		{
			// If no converter specified and type is enum, used enum values to
			// convert
			if (type.isEnum())
			{
				converterClass = type;
		...
		if (converterClass == null)
		{
			...
			converterClass = elementType != null ? findConverter((Class<? extends IStringConverter<?>>) elementType) : StringConverter.class;
		...
		try
		{
			...
			if (converterClass != null && converterClass.isEnum())
			{
				...
				{
					throw new ParameterException("Invalid value for " + optionName + " parameter. Allowed values:" + EnumSet.allOf((Class<? extends Enum>) converterClass));
				}
			}
			else
			{
				...
				if (type.isAssignableFrom(List.class) && parameterized.getGenericType() instanceof ParameterizedType)
				{
					...
					{
						// If a list converter was specified, pass the value to
						// it
					...
					{
						// No list converter: use the single value converter and
						// pass each
	...
	private IStringConverter<?> instantiateConverter(String optionName, Class<? extends IStringConverter<?>> converterClass) throws IllegalArgumentException, InstantiationException, IllegalAccessException, InvocationTargetException
	{
		...
		IStringConverter<?> result = stringCtor != null ? stringCtor.newInstance(optionName) : (ctor != null ? ctor.newInstance() : null);
